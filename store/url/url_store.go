...
func (pgs *PostgresURLStore) CreateShortURL(shortURL string, longURL string, ctx context.Context) error {
	...
	retryAttempt:
		...
		if time.Since(startTime)+sleep >= maxElapsedTime {
			pgs.logger.Errorf("CreateShortURL operation timed out after %v for %s", maxElapsedTime, shortURL)
			return ErrMaxElapsedTimeExceeded
		}
		select {
		case <-time.After(sleep):
		case <-ctx.Done():
			pgs.logger.Errorf("CreateShortURL operation cancelled during backoff for %s: %v", shortURL, ctx.Err())
			return ctx.Err()
		}
	}
	pgs.logger.Errorf("failed to create short URL after multiple retries")
...
func (pgs *PostgresURLStore) GetLongURL(shortURL string, ctx context.Context) (string, error) {
	...
	retryAttemptGetLongURL:
		...
		if time.Since(startTime)+sleep >= maxElapsedTime {
			pgs.logger.Errorf("GetLongURL operation timed out after %v for %s", maxElapsedTime, shortURL)
			return "", ErrMaxElapsedTimeExceeded
		}
		select {
		case <-time.After(sleep):
		case <-ctx.Done():
			pgs.logger.Errorf("GetLongURL operation cancelled during backoff for %s: %v", shortURL, ctx.Err())
			return "", ctx.Err()
		}
	}
	pgs.logger.Errorf("Failed to get long URL after multiple retries for %s", shortURL)
	return "", ErrRetriesExhausted
}
